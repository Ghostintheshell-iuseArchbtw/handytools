#!/usr/bin/env python3
"""
bin_report.py — Safe static triage & HTML report generator for PE/ELF binaries.
...
Features (no bypassing of protections):
  • File summary: size, hashes, type, arch, compile time, overall entropy
  • PE: directories, sections & flags, imports (best-effort), exports, debug dir presence,
        ...
        entrypoint section, overlay size
  • ELF: DT_NEEDED dependencies
  • String-based clusters for anti-debug / VM markers (indicative only)
  • Charts: overall file entropy + top section entropies per PE (if matplotlib present)
  • Single, sleek HTML with inline base64 images and embedded CSS (dark theme)
  • Optional JSON dumps (static + extended)

This tool intentionally avoids any guidance or functionality to remove/defeat software protection.
"""
...
def parse_pe_metadata(b: bytes) -> Dict[str, Any]:
    ...
    if len(b)<0x40 or b[:2]!=b"MZ": return md
    pe_off = struct.unpack_from("<I", b, 0x3C)[0]
    if pe_off + 24 > len(b) or b[pe_off:pe_off+4] != b"PE\x00\x00": return md
    ...
    try: md["compile_time_utc"]=datetime.datetime.utcfromtimestamp(timestamp).strftime("%Y-%m-%d %H:%M:%S")
    ...
    for i in range(num_sections):
        ...
        sections.append({"section_name":name,"virtual_size":virt_size,"virtual_address":virt_addr,"raw_size":raw_size,"raw_ptr":raw_ptr,"entropy":round(shannon_entropy(sec_bytes),3) if sec_bytes else None})
    md["sections"]=sections
    return md

def parse_elf_metadata(b: bytes) -> Dict[str, Any]:
    md={"is_elf":False,"class":None,"machine":None,"endianness":None,"entry_point":None}
    if len(b)>=4 and b[:4]==b"\x7fELF":
        md["is_elf"]=True
...
def parse_imports(b: bytes, peh) -> List[Dict[str,str]]:
    ...
    # IMAGE_IMPORT_DESCRIPTOR (20 bytes each), terminated by zero block
    ioff=off
    while ioff + 20 <= len(b):
...
def parse_tls_callbacks(b: bytes, peh):
    ...
    # VA list -> RVAs -> offsets to read callback array
...
HINT_KEYWORDS = {
    "debug_api": ["IsDebuggerPresent","CheckRemoteDebuggerPresent","OutputDebugString","NtQueryInformationProcess","NtSetInformationThread","ZwSetInformationThread","SetUnhandledExceptionFilter","AddVectoredExceptionHandler"],
    "time_anti": ["QueryPerformanceCounter","GetTickCount","RDTSC","rdtsc","timeGetTime"],
    "vm_markers": ["VBox","VirtualBox","VMware","KVM","QEMU","Hyper-V","VBOX","VMX","Parallels","Xen"],
    "tools_names": ["ollydbg","ida","idag","idaw","idau","windbg","x64dbg","scylla","wireshark","fiddler","procmon","procexp"],
    "sandbox": ["Sandboxie","Cuckoo","Any.Run","AnyRun","Thug","Detonator","cwsandbox","joe sandbox","joesandbox"],
    "wine": ["wine","WINEDEBUG"],
}
    ...
    if os.path.isdir(path):
        if recurse:
            ...
            for f in os.listdir(path):
                ...
                if os.path.isfile(full): targets.append(full)
    else:
        targets.append(path)
    return targets
...
def guess_binary(file: str) -> bool:
    try:
        with open(file, "rb") as f:
            hdr=f.read(4)
        return hdr[:2]==b"MZ" or hdr[:4]==b"\x7fELF"
    except Exception:
        return False
...
def analyze_file(path: str) -> Dict[str, Any]:
    ...
    ft="PE (Windows)" if data[:2]==b"MZ" else "ELF (Unix)" if data[:4]==b"\x7fELF" else "Unknown"
    pe=parse_pe_metadata(data); elf=parse_elf_metadata(data)
    markers_found=sorted({m for m in PACKER_MARKERS for s in ascii_strs if m in s})[:30]
    apis_found=sorted({api for api in API_CANDS for s in ascii_strs if api in s})
    result = {
        "path": path, "size_bytes": size, **hashes, "file_type": ft,
        "overall_entropy": round(ent,3), "pe_meta": pe if pe["is_pe"] else None,
        "elf_meta": elf if elf["is_elf"] else None,
        "packers_markers_found": markers_found, "apis_found": apis_found,
    }
    # Extended for PE/ELF
    ext = {
        "pe_directories": [], "pe_sections": [], "pe_imports": [], "pe_exports": [],
        "pe_debug_present": False, "pe_tls": {}, "pe_relocs": [], "elf_needed": [], "hint_clusters": {}
    }
    if is_pe(data):
        ...
        for s in peh["sections"]:
            ex,rd,wr = section_flags_info(s["chars"])
            ext["pe_sections"].append({"section": s["name"], "vaddr": hex(s["vaddr"]), "vsize": s["vsize"], "rptr": s["rptr"], "rsize": s["rsize"], "exec": ex, "read": rd, "write": wr})
    ...
    # String clusters (both)
    strs = ascii_strs + extract_utf16le(data,5,50000)
    ext["hint_clusters"] = cluster_hints(strs)
...
.table{width:100%;border-collapse:collapse;border-spacing:0}.table th,.table td{padding:8px 10px;border-bottom:1px solid #22273a;vertical-align:top}.table thead th{text-align:left;font-weight:600;color:var(--muted);border-bottom:1px solid #2a3150}.table tr:hover td{background:rgba(122,162,247,.06)}
...
def table(headers: List[str], rows: List[List[Any]]) -> str:
    ...
    return f'<table class="table sleek">{thead}{tbody}</table>'
...
def render_html(results: List[Dict[str,Any]], title: str) -> str:
    ...
    sum_rows=[]
    for r in results:
        pe=r.get("pe_meta") or {}; elf=r.get("elf_meta") or {}
        arch = pe.get("arch") if pe else (elf.get("machine") if elf else "")
        sum_rows.append([
            ...
            r["md5"], r["sha256"],
            pe.get("compile_time_utc",""), pe.get("num_sections",""),
            ", ".join(r.get("packers_markers_found", [])[:6]),
            ", ".join(r.get("apis_found", [])[:6]),
        ])
    files_table = table(
        ["File","Type","Arch","Size","Entropy","MD5","SHA-256","PE Compile Time (UTC)","Sections","Markers","APIs"],
        sum_rows
    )
    ...
    if HAS_MPL and results:
        ...
        try:
            ...
            plt.bar(files, ents)
            plt.xlabel("File"); plt.ylabel("Shannon Entropy (bytes)"); plt.title("Overall File Entropy")
            ...
            pass
        # PE section entropies
        for r in results:
            pe=r.get("pe_meta") or {}
            if not pe or not pe.get("sections"): continue
            pairs=[(s["section_name"], s["entropy"]) for s in pe["sections"] if s.get("entropy") is not None]
            if not pairs: continue
            pairs=sorted(pairs, key=lambda t:(t[1],t[0]), reverse=True)[:20]
            secs=[p[0] for p in pairs]; es=[p[1] for p in pairs]
            try:
                plt.figure(figsize=(9,5))
                plt.bar(secs, es)
                plt.xlabel("Section"); plt.ylabel("Entropy"); plt.title(f"Top Section Entropies — {os.path.basename(r['path'])}")
                plt.xticks(rotation=45, ha="right")
                charts_html += f'<div class="figure"><img src="{b64_png_from_fig()}" alt="Section Entropy"/><div class="cap">Top section entropies for {html_escape(os.path.basename(r["path"]))}</div></div>'
            except Exception:
                pass
    elif not HAS_MPL:
        charts_html = '<div class="small">Matplotlib not available; charts disabled. Install matplotlib to enable.</div>'

    # Aggregating extended tables across files
    # PE Directories
    dir_rows=[]
    sec_rows=[]
    tls_rows=[]
    ...
    for r in results:
        ...
        for s in ext.get("pe_sections", []):
            sec_rows.append([os.path.basename(r["path"]), s["section"], s["vaddr"], s["vsize"], s["rptr"], s["rsize"], s["exec"], s["read"], s["write"]])
        # tls
        tls = ext.get("pe_tls", {})
        if tls:
            tls_rows.append([os.path.basename(r["path"]), "yes" if tls.get("present") else "", ", ".join(tls.get("callbacks_va", []))])
        # debug presence
        dbg = ext.get("pe_debug_present", False)
        if dbg:
        ...
        # hints
        for cat, words in (ext.get("hint_clusters") or {}).items():
    ...
    secs_table = table(["File","Section","Vaddr","Vsize","Rptr","Rsize","Exec","Read","Write"], sec_rows) if sec_rows else "<div class='small'>No PE sections parsed.</div>"
    ...
    imp_table = table(["File","Module","Symbol"], imp_rows) if imp_rows else "<div class='small'>No imports parsed or imports resolved at runtime.</div>"
    ...
    nav_html = "".join([f'<a href="#{sid}">{label}</a>' for sid,label in [
        ...
        ("sections","PE Sections & Flags"),("tls","TLS & Debug"),("imports","Imports / Exports"),
        ("relocs","Base Relocations"),("elf","ELF DT_NEEDED"),("hints","Anti-Debug / VM Hints")
    ]])
...
<div class="wrapper">
  ...
  <div id="overview" class="section">
    <h2>Overview</h2>
    <div class="card">
      This report consolidates safe, static metadata for the analyzed binaries. It does not remove or defeat any protections.
      <hr class="sep">
      <div class="small">Note: timestamps and section layouts may be manipulated by protectors. Treat them as indicative, not authoritative.</div>
    </div>
  </div>
  ...
  <div id="sections" class="section"><h2>PE Sections & Flags</h2><div class="card">{secs_table}</div></div>
...
# --------------------- Main ---------------------
def main():
    ap = argparse.ArgumentParser(description="Generate a sleek HTML static-analysis report for PE/ELF binaries (no bypassing protections).")
    ...
    ap.add_argument("--json", action="store_true", help="Also write JSON outputs next to the HTML")
    ap.add_argument("--max-files", type=int, default=0, help="Limit number of binaries analyzed (0 = no limit)")
    args = ap.parse_args()
    ...
    if not targets:
        print("No PE/ELF binaries found in the given paths.", file=sys.stderr)
        sys.exit(2)
    ...
    if args.json:
        ...
        # Separate static / extended just like our ad-hoc flow
        static_list=[]
        extended_bundle={"files":[]}
        for r in results:
            rs = {k: v for k, v in r.items() if k not in ("_extended",)}
            static_list.append(rs)
            extended_bundle["files"].append({"path": r["path"], **(r.get("_extended") or {})})
